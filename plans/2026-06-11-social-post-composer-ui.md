...
## Phase 11 — Backend E2E tests (publish → simulated FB + IG)

These tests verify the composer's full API surface end-to-end against a real test DB, with **`messaging.simulation_mode`** enabled so no real Meta Graph calls fire. Pattern matches the existing `tests/e2e/` suite (pytest + `httpx.AsyncClient` over ASGI, fresh seeded DB per test).

**Reference reading before starting:**

- `tests/e2e/conftest.py` — `db_session` and `re_app_client` fixtures.
- `tests/e2e/test_vendor_onboarding.py` — example test that POSTs JSON to the API and asserts via DB queries.
- `src/services/runtime_settings.py` — `MESSAGING_SIMULATION_KEY` (`messaging.simulation_mode`) and helper to set / read it.
- `src/services/social_publish_service.py:90-112` — confirms that with `simulated=True` the service skips the Meta Graph call and writes `sim_<hex>` as `provider_post_id`.
- `src/api/posts.py` — endpoint signatures and required permission (`posts:publish`).
- How `require_permission` behaves in the E2E env (the existing tests POST to permissioned endpoints without explicit auth headers — find the seed-time user / dependency override that makes that work, and reuse it).

### Task 23: Shared E2E helper — seed Meta accounts + simulation mode

**Files:**
- Create: `tests/e2e/utils/social.py`

A small helper module that the two publishing tests share so each test stays focused on assertions, not setup.

- [ ] **Step 1: Create the helper**

```python
"""E2E helpers for social-post tests: seed Meta accounts + simulation mode."""
...
from src.services.runtime_settings import MESSAGING_SIMULATION_KEY, set_app_setting

DEFAULT_AGENCY_ID = "default"


async def enable_simulation_mode(session: AsyncSession) -> None:
    await set_app_setting(session, MESSAGING_SIMULATION_KEY, "true")
    await session.commit()


async def seed_primary_meta_accounts(session: AsyncSession) -> dict[str, str]:
    """Seed one healthy primary Facebook + Instagram account for the default agency.

    Returns a dict {"facebook": <account_id>, "instagram": <account_id>}.
    """
    ...
    facebook_account = AgencySocialAccount(
        id=uuid.uuid4(),
        connection_id=connection.id,
        agency_id=DEFAULT_AGENCY_ID,
        platform="facebook",
        provider_id="fb-page-1",
        name="Test Page",
        is_primary=True,
        authorized=True,
    ...
    instagram_account = AgencySocialAccount(
        id=uuid.uuid4(),
        connection_id=connection.id,
        agency_id=DEFAULT_AGENCY_ID,
        platform="instagram",
        provider_id="ig-account-1",
        username="test_ig",
        is_primary=True,
        authorized=True,
    ...
    session.add_all([facebook_account, instagram_account])
    await session.commit()
    return {
        "facebook": str(facebook_account.id),
        "instagram": str(instagram_account.id),
    }
```

Verify the field set against `src/models/agency_social_connection.py` and `src/models/agency_social_account.py` before running — required-vs-optional may differ from what's shown above. Adjust the seeded fields to satisfy the actual model.

If `set_app_setting` does not exist with that exact name in `src/services/runtime_settings.py`, find the helper that writes app_settings rows and use it instead (or write a small direct ORM insert for the `app_settings` table — that table is the storage backing `messaging.simulation_mode`).

- [ ] **Step 2: Verify Python compiles**

Run: `venv/bin/python -c "from tests.e2e.utils.social import enable_simulation_mode, seed_primary_meta_accounts"`
Expected: no import errors.

- [ ] **Step 3: Commit**

```bash
git add tests/e2e/utils/social.py
git commit -m "test(e2e): add social-post seeding helpers"
```

---

### Task 24: E2E test — publish-now to Facebook + Instagram (simulated)

**Files:**
- Create: `tests/e2e/test_social_posts_publish.py`

- [ ] **Step 1: Write the test**

```python
"""E2E: publish a post to Facebook + Instagram with messaging.simulation_mode on."""
...
from tests.e2e.utils.social import (
    enable_simulation_mode,
    seed_primary_meta_accounts,
)


@pytest.mark.asyncio
async def test_publish_now_to_facebook_and_instagram_simulated(
    re_app_client: httpx.AsyncClient,
    db_session: AsyncSession,
):
    await enable_simulation_mode(db_session)
    accounts = await seed_primary_meta_accounts(db_session)

    image_bytes = _make_png_bytes()
    media_response = await re_app_client.post(
        "/api/posts/media",
    ...
    assert isinstance(media_payload, list) and len(media_payload) == 1
    media_id = media_payload[0]["id"]

    publish_response = await re_app_client.post(
        "/api/posts/publish",
        json={
            "content": "Hello FB and IG #launch",
            "media_ids": [media_id],
            "account_ids": [accounts["facebook"], accounts["instagram"]],
        },
    )
    assert publish_response.status_code == 200, publish_response.text
    post_payload = publish_response.json()
    assert post_payload["status"] in {"published", "publishing"}
    target_platforms = {target["platform"] for target in post_payload["targets"]}
    assert target_platforms == {"facebook", "instagram"}

    post_id = uuid.UUID(post_payload["id"])
    fresh_post = await db_session.scalar(select(Post).where(Post.id == post_id))
    assert fresh_post is not None
    assert fresh_post.status == "published"

    targets_result = await db_session.scalars(
        select(PostTarget).where(PostTarget.post_id == post_id)
    )
    targets = list(targets_result)
    assert len(targets) == 2
    for target in targets:
        assert target.status == "published"
        assert target.provider_post_id is not None
        assert target.provider_post_id.startswith("sim_")
        assert target.error_code is None

    attempts_result = await db_session.scalars(
        select(PostPublishAttempt).where(PostPublishAttempt.post_id == post_id)
    )
    attempts = list(attempts_result)
    assert len(attempts) == 2
    assert all(attempt.simulated is True for attempt in attempts)
...
Notes:
- `_make_png_bytes` returns a tiny but valid 1x1 PNG so the backend's media validator accepts it. If `SocialPublishService.store_media_uploads` requires PIL-readable dimensions, generate the buffer with Pillow at test time instead: `from PIL import Image; buf = io.BytesIO(); Image.new("RGB", (10, 10)).save(buf, "PNG"); image_bytes = buf.getvalue()`.
- The exact column names on `PostPublishAttempt` (`simulated`, `outcome`) and the post-status enum string (`"published"`) must match `src/models/post_publish_attempt.py` and `src/constants.py`. If they differ, update the assertions, not the production code.

- [ ] **Step 2: Run the test**

Run: `venv/bin/pytest tests/e2e/test_social_posts_publish.py -v`
Expected: PASS.

If it fails, fix the production / fixture issue, not the assertion. Common adjustment points:
- Auth dependency override pattern (mirror what `test_vendor_onboarding.py` does).
- Required model fields the helper didn't set (e.g. `connection.id`, encryption columns).

- [ ] **Step 3: Commit**

```bash
git add tests/e2e/test_social_posts_publish.py
git commit -m "test(e2e): publish-now to FB+IG with simulation mode"
```

---

### Task 25: E2E test — schedule + cancel (simulated)

**Files:**
- Create: `tests/e2e/test_social_posts_schedule.py`

- [ ] **Step 1: Write the test**

```python
"""E2E: schedule a post for later, then cancel it."""
...
@pytest.mark.asyncio
async def test_schedule_and_cancel_post(
    re_app_client: httpx.AsyncClient,
    db_session: AsyncSession,
):
    await enable_simulation_mode(db_session)
    ...
    schedule_response = await re_app_client.post(
        "/api/posts/schedule",
        json={
            "content": "Scheduled post for FB + IG",
            "media_ids": [],
            "account_ids": [accounts["facebook"], accounts["instagram"]],
    ...
    post_payload = schedule_response.json()
    assert post_payload["status"] == "scheduled"
    assert post_payload["scheduled_at"] is not None
    target_platforms = {target["platform"] for target in post_payload["targets"]}
    assert target_platforms == {"facebook", "instagram"}
    ...
    for target in targets:
        ...
        assert target.provider_post_id is None

    cancel_response = await re_app_client.delete(f"/api/posts/{post_id}")
    assert cancel_response.status_code == 204, cancel_response.text

    cancelled = await db_session.scalar(select(Post).where(Post.id == post_id))
...
@pytest.mark.asyncio
async def test_cannot_cancel_already_published_post(
    re_app_client: httpx.AsyncClient,
    db_session: AsyncSession,
):
    await enable_simulation_mode(db_session)
...
The second test guards the backend's invariant that `DELETE /api/posts/{id}` only cancels `scheduled` rows (per `src/api/posts.py:98-110`). If the frontend's future Posts tab ever tries to cancel a published row, this guarantees a clear 409 instead of corrupting state.

- [ ] **Step 2: Run the test**

Run: `venv/bin/pytest tests/e2e/test_social_posts_schedule.py -v`
...
```bash
git add tests/e2e/test_social_posts_schedule.py
git commit -m "test(e2e): schedule then cancel post; reject cancel after publish"
```
...
- §14 (Testing) → Pure-logic tests via Vitest (Tasks 4, 5, 6, 7); backend E2E publishing tests via pytest with simulation mode (Tasks 23–25, added beyond the original spec on user request); manual QA covers UI behaviour (Task 26). The spec's component-level tests are deferred; deviation documented in plan intro.
